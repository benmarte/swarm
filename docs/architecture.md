...
- `SWARM_TOKEN` is a fine-grained PAT with `pull-requests: write` scope provisioned for a separate reviewer account. It is used only by the `reviewer-post` job in `pr-gates.yml` to post a real GitHub PR review — `GITHUB_TOKEN` cannot approve a PR it opened.
