    ...
    while i < len(lines):
        if lines[i].startswith('@test '):
            ...
            for n, l in ex:
                ...
                # Strip a trailing comment before judging shape, so
                # `[[ A ]] # why` cannot mask an unbound assertion.
                code = re.sub(r'\s+#.*$', '', l).rstrip()
                # A line continuing into the next is part of a larger construct;
                # judge that construct by its final line, not this one.
                if code.endswith('\\') or not code.endswith((']]', 'false', 'return 1')):
                    continue
                if re.search(r'\|\|\s*(false|return 1)$', code):
                    continue
                bad.append(f"{os.path.relpath(path, root)}:{n+1}: {code.strip()[:80]}")
            i = j
        i += 1
if bad:
